...
- Pages are **Server Components** by default and fetch via `lib/api.js`. Routes: `/` (home), `/logement/[id]` (property detail), `/favoris` (favorites), `/a-propos` (about), `/connexion` (login), `/messagerie` (messaging — **mock only**: hardcoded conversations in client state, no API/table exists for messaging). Unknown routes render `app/not-found.js` (custom 404). Each route has a co-located `page.module.css`; loading UI comes from the root `loading.js` (plus a route-level one in `logement/[id]`).
